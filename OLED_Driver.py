...
def Set_Coordinate(x, y, width=SSD1351_WIDTH, height=SSD1351_HEIGHT):
    if (x >= SSD1351_WIDTH) or (y >= SSD1351_HEIGHT):
        return
...
# color_fill_byte_arr = [0x00, 0x00] * (50)


def display_image_from_cache(buffer, x, y, width=SSD1351_WIDTH, height=SSD1351_HEIGHT):
    color_fill_byte_arr = [0x00, 0x00] * width
    if buffer is None:
        return
    # if len(color_fill_byte_arr) != 2*width:
    #     color_fill_byte_arr = [0x00, 0x00] * width
    Set_Coordinate(x, y, width, height)
    for j in range(0, height):
        for i in range(0, width):
            color_fill_byte_arr[i * 2] = ((buffer[i, j][0] & 0xF8) | (buffer[i, j][1] >> 5))
            color_fill_byte_arr[i * 2 + 1] = (((buffer[i, j][1] << 2) & 0xE0) | (buffer[i, j][2] >> 3))
        Write_Datas(color_fill_byte_arr)
